...
from models.mstcn_wrapper_grad_clipped_updated import TeCNO, MSTCNDataset
...
def load_config():
    ...
    # Add module-specific arguments with YAML values as defaults (if available)
    parser = TeCNO.add_module_specific_args(parser)
    for action in parser._actions:
        if action.dest in config:
            action.default = config[action.dest]
    
    # Parse command-line arguments (override YAML if provided)
    args = parser.parse_args()
    
    # Update args with remaining YAML values (for non-parser arguments like max_epochs)
    for key, value in config.items():
        if not hasattr(args, key):
            setattr(args, key, value)
    
    # Log key hyperparameters
    print(f"Using batch_size: {args.batch_size}, learning_rate: {args.learning_rate}, "
          f"max_epochs: {args.max_epochs}, seed: {args.seed}, "
          f"mstcn_stages: {args.mstcn_stages}, mstcn_layers: {args.mstcn_layers}")
    return args
...
def main():
    ...
    weights = 1.0 / (class_counts + 1e-6)  # Unnormalized weights
    print(f"Training class counts: {class_counts}")
    print(f"Class weights: {weights}")
    dataset_weights = {"train": weights}

    # Log class distributions for validation and test
    for split in ["val", "test"]:
        class_counts = {}
        for file_name in dataset[split].file_list:
            label_name = file_name.split('_')[-1].replace('.npy', '')
    ...
    logger = [
        TensorBoardLogger(save_dir=args.log_dir, name="mstcn"),
