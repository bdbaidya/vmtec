...
class MSTCNDataset(Dataset):
    def __init__(self, folder_path, label_map, oversample_rare=False):
        ...
        if oversample_rare:
            ...
            for f in self.file_list:
                label = f.split('_')[-1].replace('.npy', '')
                weight = 3 * total_count / (len(class_counts) * class_counts[label])
                factor = max(1, int(weight))
                oversampled_files.extend([f] * factor)
            self.file_list = oversampled_files
            #logging.info(f"Oversampled file list: {len(self.file_list)} files, Original counts: {class_counts}")
...
class TeCNO(LightningModule):
    ...
    def log_metrics(self, outputs, step="val"):
        ...
        avg_precision = torch.stack(precision_list).mean()
        avg_recall = torch.stack(recall_list).mean()
        avg_f1 = torch.stack(f1_list).mean()
        avg_dice = torch.stack(dice_list).mean()
    ...
    def __dataloader(self, split=None):
        ...
        if split in ["val", "test"]:
            ...
            for f in dataset.file_list:
                label = f.split('_')[-1].replace('.npy', '')
                class_counts[label] = class_counts.get(label, 0) + 1
            weights = [1.0 / class_counts[f.split('_')[-1].replace('.npy', '')] for f in dataset.file_list]
    ...
    def train_dataloader(self):
        ...
        logging.info(f"Training data loader called - size: {len(dataloader.dataset)}, sampler: {type(dataloader.sampler).__name__}")
        return dataloader
